 ...
 */
WINGSLDLL_API gsl_dht * gsl_dht_alloc(size_t size);
WINGSLDLL_API gsl_dht * gsl_dht_new(size_t size, double nu, double xmax);
 ...
 */
WINGSLDLL_API int gsl_dht_init(gsl_dht * t, double nu, double xmax);

/* The n'th computed x sample point for a given transform.
 * 0 <= n <= size-1
 */
WINGSLDLL_API double gsl_dht_x_sample(const gsl_dht * t, int n);


/* The n'th computed k sample point for a given transform.
 * 0 <= n <= size-1
 */
WINGSLDLL_API double gsl_dht_k_sample(const gsl_dht * t, int n);


/* Free a transform object.
 */
WINGSLDLL_API void gsl_dht_free(gsl_dht * t);


/* Perform a transform on a sampled array.
 * f_in[0] ... f_in[size-1] and similarly for f_out[]
 */
WINGSLDLL_API int gsl_dht_apply(const gsl_dht * t, double * f_in, double * f_out);
