/***********************************************************
 * SX11 Sample Application 3.
 * Copyright (C) 1998-2001 Sensoray
 *
 ...
 *
 * The method implemented herein makes use of interrupts.
 * The interrupt service procedure Service restarts the frame
 * grabber and posts the message to the application's message
 * queue requiring a window update.
 ************************************************************/
...
DWORD Service (void);		                // Interrupt service procedure
...
BUFFER buffer[2];                           //Buffer structure array
HFG hfg;                                    //Frame grabber handle
INT_DATA intdata;							//interrupt structure
DWORD timer0, timer, frames;                //To calculate FPS rate
int current;
/***************************************/
...
ECODE FrameGrabber (DWORD fgcommand)
{
    ...
    switch (fgcommand) {
        ...
        case FG_INIT:
            ...
            intdata.hfg = hfg;
            intdata.func = Service;
            intdata.priority = THREAD_PRIORITY_TIME_CRITICAL; //fastest
            intdata.mask = STATUS_READY;
            //Start acquisition
            if ((ecode = X11_InterruptOn (&intdata))) {
                return ecode;
            }
            X11_InterruptUnmask (&intdata);
            timer0 = clock();
            acqflag = TRUE;
...
DWORD Service (void)
{
    ...
    X11_StartAcquire (hfg, buffer[current].hbuf, AMODE_SINGLE);
    X11_InterruptUnmask (&intdata);
    //Update FPS rate value
    frames++;
    timer = clock() - timer0;
    sprintf (str, "X11 Sample3;   FPS=%4.1f\0", (float) frames * CLOCKS_PER_SEC / timer);
	SendMessageTimeout (hwnd, WM_SETTEXT, 0, (LPARAM) str, SMTO_NORMAL, 200, NULL);
	SendMessageTimeout (hwnd, WM_PAINT, 0, 0, SMTO_NORMAL, 200, NULL);
    return 0;
}
