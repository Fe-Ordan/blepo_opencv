...
/* Blepo note:  
   I copied this file from gsl/rng/mt.c, changed the extension to .cpp (so it would
   recognize commands such as inline), and changed the include files below.  No other
   changes were made.  It should not have been necessary for me to include this file 
   directly at all, but I was running into linking problems with specific random number 
   generators, like gsl_rng_mt19937.  It appears that WinGsl neglected to include these 
   variables in its .lib file.  -- Stan Birchfield, 6/13/05.  
*/
