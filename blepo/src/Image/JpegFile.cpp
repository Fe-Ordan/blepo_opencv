...
BYTE * JpegFile::JpegMemoryToRGB(const unsigned char* input, size_t input_size,
							   UINT *width,
							   UINT *height)

{
	...
	*/
	...
	jpeg_memory_src(&cinfo, (const JOCTET*) input, input_size);
...
// added by Neeraj Kanhere 11/3/09
size_t JpegFile::RGBToJpegMemory(BYTE *dataBuf,					// RGB buffer
						  BYTE* write_buffer,
...

{
  ...
  if (height==0)
  {
	...
	//jpeg_stdio_dest(&cinfo, outfile);
  jpeg_memory_dest(&cinfo, write_buffer, &nbytes); 
